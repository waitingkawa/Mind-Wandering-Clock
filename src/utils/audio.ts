...
class RelaxationAudioEngine {
  ...
  public playChime() {
    ...
    // A short C-major music-box phrase: C6, E6, G6, C7.
    // Each note combines a warm triangle body with a quiet bell overtone.
    const melody = [
      { frequency: 1046.5, offset: 0 },
      { frequency: 1318.51, offset: 0.18 },
      { frequency: 1567.98, offset: 0.36 },
      { frequency: 2093, offset: 0.58 },
    ];

    const chimeBus = this.ctx.createGain();
    chimeBus.gain.setValueAtTime(0.48, startTime);
    ...
    melody.forEach(({ frequency, offset }, index) => {
      ...
      const duration = index === melody.length - 1 ? 1.45 : 0.85;

      const body = this.ctx.createOscillator();
      body.type = 'triangle';
      body.frequency.setValueAtTime(frequency, noteStart);

      const sparkle = this.ctx.createOscillator();
      sparkle.type = 'sine';
      sparkle.frequency.setValueAtTime(frequency * 2.01, noteStart);

      const noteGain = this.ctx.createGain();
      noteGain.gain.setValueAtTime(0.0001, noteStart);
      noteGain.gain.exponentialRampToValueAtTime(0.25, noteStart + 0.012);
      ...
      sparkleGain.gain.exponentialRampToValueAtTime(0.055, noteStart + 0.008);
      sparkleGain.gain.exponentialRampToValueAtTime(0.0001, noteStart + duration * 0.55);

      body.connect(noteGain);
      sparkle.connect(sparkleGain);
      noteGain.connect(chimeBus);
      sparkleGain.connect(chimeBus);

      body.start(noteStart);
      sparkle.start(noteStart);
      body.stop(noteStart + duration);
      sparkle.stop(noteStart + duration);
    });
  }
